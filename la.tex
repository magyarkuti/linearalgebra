...
amiket majd később vezetünk be.
...
\end{proposition}
\begin{proof}
    Tekintsük a $p$ polinomot mint a komplex számtest feletti polinogyűrű egy elemét,
    és alkalmazzuk a komplex polinomfaktorizációról szóló tételt.
...
\end{proof}
A fenti állitásból két dolog azonnal látszik.
Az első, 
...
\scwords A lineáris algebra kezdő fejezetéhez érkeztünk, miután áttekintettük azokat az általános algebrai ismereteket,
amelyek nélkül nem tárgyalhatók a lineáris algebra beli gondolatok.
...
\begin{proposition}
    ...
    azonos a homogén rendszer megoldáshalmazának egy partikuláris megoldással való 
    eltoltjával.
\end{proposition}
\begin{proof}
    Legyen először $x$ egy megoldás, azaz $Ax=b$. 
    Mivel $x_0$ is egy megoldás, ezér $Ax_{0}=b$ is teljesül.
...
emiatt mindegy melyik partikuáris megoldást rögzítettük.
...
\begin{proposition}
    ...
    azaz a $b$ vektor az $A$ mátrix oszlop vektoréhez tartozik.
\end{proposition}
...
\]
\begin{proposition}
    Az $Ax=b$ inhomogén lineáris egyenlterendszernek pontosan akkor van egyetlen 
...
akkor az azt jelenti, hogy az első sorba eliminináltuk az $j$-edik ismeretlent, 
...
hogy a bal oldali $m\times m$ méretű mátrix válljon az identitás mátrixszá, 
...
A transzformáció során a Gauss-Jordan eliminációt használtuk, amely a szakasz elején említett 1) és 2) tipusú mátrixokkal mint balszorzásokkal hajtható végre.
Az algoritmus végén még sorokat is felcseréltünk, ami egy 3) tipusú balszorzást jelent.
Ha az 1) vagy 2) vagy 3) tipusú mátrixszot \emph{elemi mátrixnak}\index{elemi mátrix}
nevezünk, 
...
Megmutattuk, hogy négyzetes mátrixok gyűrűjében ha két mátrix szorzata a gyűrű
egységeleme, akkor a mátrixok kommutálnak.\index{kommutáló mátrixok}
...
\begin{proposition}
    Egy végesen generát altér minden altere is végesen generált.
...
\scwords A koordinátatér fogalmát vezetjük be. 
...
\begin{defprop}
    ...
    Az $L\left( V,W \right)$ lineáris operációk halmaza a szokásos függvényműveletekkel vektorteret alkot.
\end{defprop}
A jelölések tehát:
\[
...
Itt jegyezem meg, hogy a tradiciókat követve egy $A$ lináris operáció esetén az $x$ vektor
$A\left( x \right)$ képét a zárójeleket elhagyva $Ax$ módon írjuk.

\begin{proposition}
    Legyen $A\in L\left( V,W \right)$ egy lineáris operátáció.
    Ekkor
    \begin{enumerate}
    ...
    \end{enumerate}
\end{proposition}
Ezek szerint egy lineáris bijekció bázist bázisra képez.
\begin{definition}[izomorf vektorterek]
    ...
    Az ugyanazon test feletti $V,W$ vektortereket egymással izomorf vektortérnek mondjuk,
...
\begin{proof}
    ...
    hogy $\Psi$ egy lineáris operátáció,
    a bázis lineárisan függetlenségét használva adódik $A$ injektivitása,
    a szürjektivitás pedig a bázis generátorrendszer tulajkdonságát használva igazolható.
\end{proof}
...
\end{definition}
Pont azt muattuk meg, hogy egy rögzített bázis mellett az a leképezés,
amely egy vektorhoz hozzárendeli a bázisban felírt koordinátáit, egy izomorfiz
a vektortér és a koordinátatere közt.
...
\chapter{Alterek direkt összege}
...
\begin{proof}
    ...
    \footnote{
        ...
        Az összes $r$ elemet tartalmazó szumma iyenkor üres, tehát értéke zérus.
    }
    Nyilvánvaló, hogy az
    \[
...
\end{proof}
\begin{definition}[direktösszeg]
    Legyenek az $M_1,\dots,M_s\subseteq V$ alterek a $V$ vektortér alterei ($s\geq 2$).
    Azt mondjuk, hogy az $N\subseteq V$ altér az $M_1,\dots,M_s$ alterek \emph{direktösszege}\index{direktösszeg},
    ha
    \begin{enumerate}
...
\end{definition}
Külön érdemes figyelni az $s=2$, tehát csak két altér direktösszegének esetére.
...
\end{proposition}
\begin{proof}
    Tegyük fel, hogy $M$ direktösszege az $M_1,\dots,M_s$ altereknek.
...
azt igazoltuk, hogy $N$ pontosan akkor az $M_1$ és $M_2$ direktösszege,
...
$M_1\oplus\dots\oplus M_s$ direktösszeg nem függ az alterek sorrendjétől,
2. előnye pedig,
hogy ha képeznünk kell az $M_1,\dots,M_s$ alterek direktösszegét, 
akkor rekurzívan járhatunk el: 
ha a korábbi lépésben ellenőriztük, hogy az $M_1\oplus\dots\oplus M_{n-1}$ értelemes, akkor 
a következő lépésben csak azt kell ellenőrinünk, 
hogy $M_n$ diszjunkt az első $n-1$ altér direktösszegétől.
...
Last Modified : Tue 19 Nov 2019 03:58:07 PM CET
