...
\section{Az alaptétel következményei}
\subsection{Polinom faktorizáció}
Láttuk, hogy ha $\alpha \in \mathbb{C}$ komplex szám gyöke valamely $p\left( z\right) $ komplex polinomnak, akkor a gyöktényező kiemelhető, azaz $p\left( z\right) =\left(
z-\alpha \right) q\left( z\right) $ alakú, ahol $q\left( z\right) $
polinom foka legalább eggyel kisebb mint $p\left( z\right) $ polinom foka.
Ebből, és abból a tényből, hogy ha $q$ nem konstans polinom,
akkor az algebra alaptétele miatt létezik gyöke, teljes indukcióval
könnyen következik az alábbi állítás.

\begin{proposition}
\label{Th:PolFact}Legyen $p:\mathbb{C}\rightarrow \mathbb{C}$ komplex $n$ -ed fokú polinom.
Ekkor e polinom felbomlik legfeljebb elsőfokú komplex
polinomok szorzatára.
Azaz léteznek nem feltétlen különböző $c_{1},c_{2},\ldots c_{n}$ komplex számok, valamint $a$
komplex szám, hogy 
\begin{equation}
p\left( z\right) =a\left( z-c_{1}\right) \left( z-c_{2}\right) \ldots \left(
z-c_{n}\right) 
\end{equation}
minden $z\in \mathbb{C}$ esetén.
\end{proposition}

Most tekintsük a valós együtthatós $p$ polinom esetét.
Nyilván,
minden $z\in \mathbb{C}$ mellett $\overline{p\left( z\right) }=p\left( 
\overline{z}\right) ,$ így ha valamely $z$ komplex szám gyöke $p$
-nek, akkor $\overline{z}$ szintén az.
Ha most $p$ polinomot komplex
polinomnak tekintjük, akkor az (\ref{Th:PolFact}) szorzat-előállításban a 
$c_i$ gyökök mindegyike vagy valós, vagy van valamely $j$,
melyre $c_i=\overline{c_j}$.
Ilyen módon a $p$ polinom 
\[
p\left( z\right) =a\left( z-c_1\right) \left( z-c_2\right) \ldots \left(
z-c_k\right) \left( z-d_1\right) \left( z-\overline{d_1}\right) \ldots
\left( z-d_s\right) \left( z-\overline{d_s}\right) 
\]
alakú, ahol $c_i\in \mathbb{R},d_i\in \mathbb{C}$ , $i=1\ldots k,$ $j=1\ldots s,$
$k+2s=n$.
De világos, hogy $d\in \mathbb{C}$ -re 
\[
\left( x-d\right) \left( x-\overline{d}\right) =x^2-x\overline{d}-dx+d\overline{d}=x^2-x\left( \overline{d}+d\right) +d\overline{d}.
\]
valós együtthatós másodfokú polinom.
Ebből következik az
alábbi állítás.

\begin{proposition}
\label{Th:RealPolFact}Legyen $p:\mathbb{R}\rightarrow \mathbb{R}$ valós együtthatós $n$ -ed fokú polinom.
Ekkor e polinom felbomlik
legfeljebb másodfokú valós polinomok szorzatára.
Azaz léteznek nem feltétlen különböző $c_{1},c_{2},\ldots c_{k}$
valós számok, $a$ valós szám, és $q_{1},q_{2},\ldots
q_{s}$ valós együtthatós pontosan másodfokú polinomok, 
úgy hogy $k+2s=n$, valamint 
\[
p\left( x\right) =a\left( x-c_{1}\right) \left( x-c_{2}\right) \ldots \left(
x-c_{n}\right) q_{1}\left( x\right) q_{2}\left( x\right) \ldots q_{s}\left(
x\right) 
\]
minden $x\in \mathbb{R}$ mellett.
\end{proposition}

\subsection{A komplex számok egyértelműsége}
...
Last Modified : Sun 15 Sep 2019 08:57:44 AM CEST
