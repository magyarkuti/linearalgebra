...
\begin{proposition}[Euklidesz]\index{Euklideszi algoritmus}
    ...
    Ennek a $p_s$ polinomnak a normáltja a $p\text{ és a }q$ polinomok legnagyobb közös osztója.
\end{proposition}
\begin{proof}
    ...
    hogy ha $h|p_{i-1}$ és $h|p_i$, akkor $h|p_{i+1}$ is fennáll.
    Legyen most $d|p$ és $d|q$, 
    azaz $d|p_{-1}$ és $d|p_0$. 
    Az iménti gondolatot alkalmazva $d|p_1$ is igaz.
    De akkor $d|p_0$ és $d|p_1$ miatt $d|p_2$. 
    Ezt folytatva azt kapjuk, hogy $d|p_s$.

    Ez éppen azt jelenti, 
    hogy a $p_s$ polinomot a főegyütthatójával elosztva kapjuk a $p$ és a $q$ polinomok legnagyobb közös osztóját.
\end{proof}
...
\end{proposition}
\begin{proof}
    Ha fennáll a fenti azonosság,
    és $d|p,d|q$, akkor $d|1$, ami azt jelenti, hogy $1$ a két polinom legnagyobb közös osztója.

    A Bezout--azonosság igazolásához érdemes bevezetni a lövetkező jelölést.
    Ha $r_1,r_2$ valamely polinomok, akkor jelölje
    \[
        J\left( r_1,r_2 \right)=\left\{ fr_1+gr_2:f,g\in\mathbf{F}\left[ t \right] \right\}.
    \]
    Világos, hogy ha $r_3,r_4\in J\left( r_1,r_2 \right)$, akkor 
    $J\left( r_3,r_4 \right)\subseteq J\left( r_1,r_2 \right)$.
    Na most, ha a $p$ és $q$ polinomok relatív prímek,
    akkor az Euklideszi-algoritmust e két polinomra felírva,
    kapjuk a $p_{-1}, p_0,p_1,\dots,p_s,p_{s+1}$ polinomok véges sorozatát,
    ahol $p_s\left( t \right)=\alpha\neq 0$ és $p_{s+1}=0$.
    E polinokat definiáló (\dag) azonosság szerint,
    \(
    ...
    \[
        J\left( p,q \right)=
        J\left( p_{-1},p_0 \right)\supseteq
        J\left( p_0,p_1 \right)\supseteq
        \dots\supseteq
        J\left( p_{s-2},p_{s-1} \right)\ni p_s.
    \]
    Azt láttuk be, hogy léteznek $f,g\in\mathbf{F}\left[ t \right]$ polinomok, hogy
    $f\left( t \right)p\left( t \right)+g\left( t \right)q\left( t \right)=\alpha.$
    Ezt az egyenlőséget az $\alpha$ számmal osztva készen is vagyunk.
\end{proof}
...
Last Modified : Tue 13 Aug 2019 05:44:06 AM CEST
