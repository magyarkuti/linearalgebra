...
\begin{proposition}\label{pr:polinomfv}
    Tegyük fel, hogy a $p,q\in\mathbb{F}\left[ t \right]$ polinomok legfeljebb $n$-edfokúak, ahol $n$ egy nemnegatív egész,
    és tegyük fel, 
...
\begin{proposition}[polinom faktorizáció]\label{pr:polfact}
    Teszőleges test feletti polinomgyűrűben, 
...
\begin{proof}
    ...
    A polinomgyűrű nullosztómentessége szerint az első polinomokkal egyszerűsíthetünk, ergo
    \[
...
\begin{definition}[mátrix]
    ...
    Emiat két mátrix akkor azonos, ha azonos méretűek, és a megfelelő elemeik is azonosak.
...
\end{definition}
Az azonos tipusú mátrixok közt műveleteket definiálunk:
\begin{definition}[mátrixok összege]\index{mátrixok összege}
    Rözített $m,n\in\mathbf{N}$ mellett, ha $A,B\in\mathbf{F}^{m\times n}$,
...
Most defináljuk egy számnak és egy mátrixnak a szorzatát.
...
\end{definition}
Az összeadás és szorzás műveleteket a dusztributivitás kapcsolja össze:
\begin{proposition}
...
\begin{proof}
    ...
    \end{multline*}
    A testben fenálló asszociativitás és kommutativitás miatt a bal- és a jobboldali kifejezés azonos.
\end{proof}
\begin{proposition}
...
de ez enélkül is nagyon egyszerűen látszik.%
\footnote{
    Ha 
    \(
        \begin{pmatrix}
...
hogy egy testbeli $a$ elemet $[a]$ módon írjuk. A szorzás és az összeadás definiciója ugyanazt adja,
...
\begin{definition}[Izomorf testek]\index{izomorfizmus}
    ...
    Aztmondjuk, hogy a két test \emph{izomorf} egymással, 
...
A $2\times 2$-es valós mátrixok egységelemes gyűrűjében tahát $\mathcal{R}$ egy olyan részgyűrű, ami még test is,
és izomorf az $\mathbf{R}$ valósszámtesttel.
...
\begin{defprop}[komplex számtest]
    ...
    \)
    mátrixot.
    Tekintsük az ilyen tipusú mátrixok $\mathcal{C}$-vel jelölt halmazát:
    \[
        \mathcal{C}
        =
        \left\{ 
...
\begin{proof}
    ...
    \[
        ...
        M_{a+c,b+d}
    \]
    és hasonlóan a mátrix szorzás definicója szerint
    \[
        M_{a,b}\cdot M_{c,d}=
        \begin{pmatrix}
            a&-b\\
...
\(
    ...
    \end{pmatrix}
\)
elem olyan, hogy a négyzete a szorzásra nézve repordukáló elemnek az összeadásra nézve képzett inverze:
\[
    M_{0,-1}\cdot M_{0,-1}
    =
    \begin{pmatrix}
...
mert ha a szokásoknak megfelelően a test multiplikatív neutrális elemét az $1$ szimbolúmmal jelöljük, 
akkor olyan elemet találtunk a komplex számok testjében, 
...
\(
    ...
    \end{pmatrix}
\)
mátrixra gondolunk,\footnote{Kicsit pontosabban: a valós számok mátrix $2\times 2$-es mátrixreprezentációját használjuk.}
...
Ez a normálalak reprezentáció mellett azt jelenti, hogy az összedás definíciója csak
\[
...
Más nem is lehet:
Az összedáshoz (\dag) csak el kell végezni a műveletet majd kimelni $i$-t,
 a szorzás definiciójához (\ddag) 
 pedig a disztributív szabály elvégzése után figyelembe véve, hogy $i^2=-1$,
...
Last Modified : Sat 07 Sep 2019 09:18:06 PM CEST
